...
対処としては、その局だけKIFをあきらめて、118手ぶんの全記録をテキストにして送りました。反則の入った対局を普通の棋譜ファイルで渡せないというのは気持ちの悪い話なので、さっきの入力ツールのほうに「反則手の手前で自動的に分割して書き出す」を宿題として積んであります。

## お渡ししたもの

最終的に、6局ぶんの棋譜をメールでお送りしました。二歩で途中から読めなくなる1局だけは、118手ぶんの全記録をテキストにした形でお渡ししています。
