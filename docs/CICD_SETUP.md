...
- **S3バージョニング有効**: コンソールで各オブジェクトの前バージョンを復元。GHAデプロイも
  `aws s3 sync` で上書きするだけなので、旧バージョンは自動的にS3側へ残る（本番バケット共通の
  実質的なバックアップ）。復元は `local-claude` プロファイル等、`s3:ListBucketVersions` /
  `s3:GetObjectVersion` を持つ人が手動で行う（CIロールにはこの権限を持たせていない — 最小権限）
- **デプロイ前の安全確認**: GHAの `Pre-deploy safety check` ステップで、デプロイ直前の本番
  オブジェクト数が80件未満なら異常とみなしてデプロイを中断する（`deploy.sh` と同じ閾値）
- **ローカルバックアップ**: `~/shogi_prod_backups/prod-<日時>/`（`./deploy.sh` 実行時に自動取得）
- **緊急時**: `./deploy.sh` でローカル状態を直接反映

補足: GHAの成果物（Actions artifact）に本番ファイルをアップロードする方式は採用していない。
このリポジトリは**public**で、本番ファイルには実APIキーが埋め込まれているため、artifactとして
上げると誰でもダウンロードできてしまいキー漏洩になる。バックアップはS3バージョニング（既に
Enabled）に任せ、GHA側はデプロイ前の簡易な健全性チェックのみ行う設計にしている。
